...
public class Ip implements BaseDao {
    ...
    public SearchSourceBuilder initTemple(DaoArgs args) {
        ...
        CardinalityBuilder ip = AggregationBuilders.cardinality("ip").field("ip").precisionThreshold(1000);
        TermsBuilder city = AggregationBuilders.terms("city").field("geoip.city_name").size(30).order(Terms.Order.aggregation("1",false));
        city.subAggregation(ip);
        CardinalityBuilder ipFirst = AggregationBuilders.cardinality("ipFirst").field("ip").precisionThreshold(1000);
        ...
        region.subAggregation(city).subAggregation(ipFirst);
        CardinalityBuilder ipParent = AggregationBuilders.cardinality("ipParent").field("ip").precisionThreshold(1000);
        ...
        country.subAggregation(ipParent).subAggregation(region);
//    todo    select.must()
